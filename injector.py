...
parser.add_argument('-m', metavar='(1 or 2))',
                    help='Method 1:-Injecting into bytecode  (Preferred method) '
                         'Method 2:-Injecting as dependency to a native library (JNI)')
...
def smali_inject(mainactivity):
    ...
    for line in smalicontent:
        if "# direct methods" in line:
            ...
            smalicontent[smalicontent.index(
    ...
    if isdir:
        ...
        if arch_name in abislist:
            ...
            if arch_name == "armeabi-v7a":
                copyfile("gadget/arm/libfrida-gadget.so",
                         Path(outputdirectory + "/lib/" + arch_name + "/libfrida-gadget.so"))
            elif arch_name == "arm64-v8a":
                copyfile("gadget/arm64/libfrida-gadget.so",
                         Path(outputdirectory + "/lib/" + arch_name + "/libfrida-gadget.so"))
            else:
                copyfile("gadget/" + arch_name + "/libfrida-gadget.so",
                         Path(outputdirectory + "/lib/" + arch_name + "/libfrida-gadget.so"))
        ...
        while True:
            try:
                archinput = int(
                    input("Select Mobile architecture \n1)All\n2)armeabi-v7a\n3)arm64-v8a\n4)x86\n5)x86_64\n  :-"))
                break
            except ValueError:
        ...
        if archinput == 1:
            for i, j in zip(archdata, abislist):
                ...
                copyfile("gadget/" + i + "/libfrida-gadget.so",
                         Path(outputdirectory + "/lib/" + j + "/libfrida-gadget.so"))
        elif archinput == 2:
            os.mkdir(outputdirectory + "/lib/" + abislist[0])
            copyfile("gadget/arm/libfrida-gadget.so",
                     Path(outputdirectory + "/lib/" + abislist[0] + "/libfrida-gadget.so"))
        elif archinput == 3:
            os.mkdir(outputdirectory + "/lib/" + abislist[1])
            copyfile("gadget/arm64/libfrida-gadget.so",
                     Path(outputdirectory + "/lib/" + abislist[1] + "/libfrida-gadget.so"))
        elif archinput == 4:
            os.mkdir(outputdirectory + "/lib/" + abislist[2])
            copyfile("gadget/x86/libfrida-gadget.so",
                     Path(outputdirectory + "/lib/" + abislist[2] + "/libfrida-gadget.so"))
        elif archinput == 5:
            os.mkdir(outputdirectory + "/lib/" + abislist[3])
            copyfile("gadget/x86_64/libfrida-gadget.so",
                     Path(outputdirectory + "/lib/" + abislist[3] + "/libfrida-gadget.so"))
...
def install_apk():
    ...
    if (install_input == "y") | (install_input == "yes"):
        ...
        if "product" in devices:
            ...
            print(Style.RESET_ALL)
            subprocess.call(
                ["frida", "--codeshare", "pcipolloni/universal-android-ssl-pinning-bypass-with-frida", "-U", "Gadget"])
        else:
...
def inject_native():
    ...
    if isdir:
        ...
        selectedlib = input(
            "Paste the name of  One of the Native Library to Inject Hope that the library is loaded early ")
